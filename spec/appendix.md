...
| Requirement | This specification requires | Reported behaviour | Resolution |
|---|---|---|---|
| VTI-VTA-004, VTI-VTA-005 *(observed)* | A signing oracle parses and constrains what it signs, and refuses opaque octets | Three caller classes share one general path that signs an arbitrary octet string. The task-signing path constrains its input — envelope shape, issuer against the entry's principal, no prior proof, expiry. An in-process proof constructor presents canonicalised octets it has already constrained, one layer above the oracle. An externally reachable operation accepts an octet string from any authorized caller with no structural constraint at all, subject only to key and context authorization | Constrain the externally reachable path, which means either parsing what it is given or applying domain separation to what it produces. Both change what verifiers see, so this is a breaking change to a published operation and a decision for the maintainer rather than a repair. Key-level and context-level authorization bound *which key* signs, never *what is signed*, and that is the distinction the requirement turns on |
