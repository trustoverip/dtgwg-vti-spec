...
| Requirement | This specification requires | Reported behaviour | Resolution |
|---|---|---|---|
| VTI-VTA-004, VTI-VTA-005 *(observed, resolution in review)* | A signing oracle parses and constrains what it signs, and refuses opaque octets | Three caller classes share one general path that signs an arbitrary octet string. The task-signing path constrains its input — envelope shape, issuer against the entry's principal, no prior proof, expiry. An in-process proof constructor presents canonicalised octets it has already constrained, one layer above the oracle. An externally reachable operation accepts an octet string from any authorized caller with no structural constraint at all, subject only to key and context authorization | **Domain separation**, chosen over parsing what is presented. The externally reachable path frames the payload under a versioned tag before signing, so the signature verifies as a payload of that operation and as nothing else. The in-process proof constructor is *excluded by name*: its bytes carry a meaning its own specification has established, and framing them again produces a proof a conforming verifier rejects — so the domain is an argument every call site states rather than a prefix applied to all of them. Breaking for verifiers of previously produced signatures, deliberately and without a compatibility mode, since one would restore what is being removed |
...
| VTI-AUD-004 *(observed, in progress)* | The audit trail is tamper-evident | One node type chains each entry to its predecessor by digest; another's default sink is flat and unchained, so a compromised node can rewrite its own history undetectably. The pluggable sink admits a chained backend; the default is not one | Chain by default, reusing the chaining the other node type already runs rather than adding a second scheme. Settled while preparing it: the chain's first entry is the creation of the key that chains it, so a verifier reading from the start learns which key the following entries are hashed under from an entry hashed under that same key; and rotation takes the same shape, each key's segment opening with the record of why the key changed |
| VTI-AUD-005 *(observed, in progress)* | An audit record refers to personal data rather than embedding it, so erasure leaves the chain verifiable | Records embed subject identifiers directly. Where the log is chained, an erasure breaks the chain; where it is not, the erasure is undetectable | Commit to the identifier under a keyed hash and keep the plaintext beside it, so an erasure nulls the plaintext while the commitment keeps the row correlatable and the chain digest — which excludes the plaintext members — still verifies. **The key is randomly generated rather than derived from the node's master seed**: derivation would make the recovery phrase an audit-key backup, so an erasure would be reversible by brute force, over the small candidate space of identifiers that node has seen, for anyone holding it. Still the entry most worth resolving before a deployment carries personal data at scale, because it cannot be fixed retroactively for records already written |
