...
| VTI-OPS-020, VTI-OPS-021, VTI-OPS-024, VTI-OPS-093 *(observed)* | Every operation document carries a proof by its issuer and a time of issue, on every binding, and a binding does not relax either on the strength of the transport's own authentication | At one community node, 47 tasks whose own definitions declare the proof REQUIRED are served only over an HTTP binding that carries no operation document at all. The body is the bare payload, the task is named in a header and checked against the route, and the caller is authenticated by a bearer session token. No proof is required or verified, no time of issue is checked, although every one of these definitions except six also declares it REQUIRED, and no document identifier enters the replay record. There are 30 community tasks. Administrative: `vtc/members/{credentials, update, admin-remove, purge}`, `vtc/join-requests/decide`, `vtc/admin/invites/{create, revoke}`, `vtc/community/profile/update`, `vtc/config/{export, import}`, `vtc/backup/{export, import}`, `vtc/registry/sync-jobs/{retry, discard}`, `vtc/endorsement-types/{register, delete}` and `vtc/website/{files/delete, rollback}`. Taken under a member's session: `vtc/members/{renew, rotate-challenge, rotate, self-remove, personhood/assert, personhood/revoke}`, `vtc/relationships/revoke`, `vtc/endorsements/{issue, revoke}`, `vtc/invitations/{issue, revoke}` and `vtc/vetting/vetters/grant`. There are 17 canonical tasks shared with other node types: `acl/{grant, change-role, revoke}`, `config/{patch, reload, restart}`, `policy/{upsert, activate}`, `audit/list`, `auth/{whoami, sessions/list, revoke-session}` and `auth/passkey/{list, enroll/start, enroll/finish, revoke/start, revoke/finish}`. The six that do not declare the time of issue REQUIRED are `vtc/config/export`, `auth/whoami`, `auth/sessions/list`, `auth/passkey/list`, `auth/passkey/enroll/start` and `auth/passkey/revoke/start`. One more, `vtc/auth/admin-session`, is unauthenticated at the transport; it takes an access token in the body and also carries no proof. Three of the community tasks are also served by the node's document dispatcher (see the next entry) | Bind each of these tasks in the document dispatcher, which accepts a signed operation document, verifies its proof, checks its time of issue and records its identifier. Keep the bearer route only as a documented transitional path with a stated removal point. A token authenticates whoever presents it and not the party who composed the request, so it cannot satisfy a definition that requires a proof |
| VTI-OPS-021, VTI-OPS-024 *(observed)* | A document for a task that requires a proof is refused without one on every binding, and a document outside the acceptance window is refused | The same node's document dispatcher verifies a proof whenever one is present. It does not itself refuse a document without one when the task's definition declares the proof REQUIRED: attribution is taken from the proof when there is one, and otherwise from the transport's authentication of the sender. Joining over the messaging transports depends on this, because those documents carry no proof. The tasks affected are `vtc/join-requests/{submit, status, withdraw, supplement}`, `vtc/members/{self-remove, vmc, personhood/assert}` and `vtc/vetting/{revoke-statement, vetters/grant}`. The dispatcher also applies no acceptance window to the time of issue, and does not refuse a document that states none when the task requires one. This is an observed instance of the reported VTI-OPS-021 entry above | On every binding, refuse a proofless document for a task whose definition requires a proof, and refuse a document without a time of issue, or with one outside the acceptance window, for a task that requires it. This has to be done before the previous entry's resolution closes anything. Moving a task into a dispatcher that does not enforce its proof requirement moves the divergence instead of closing it |
| VTI-OPS-110, VTI-OPS-051 *(observed)* | A node does only what the definition of the task it serves defines, and a step-up rule is enforced by the policy gate rather than by a handler | The definition of `vtc/members/update` refuses `role: admin` with `vtc/members/update:adminRoleForbidden` and leaves promotion to a separate, gated operation. One community node instead promotes to admin through that task and never emits the code. The promotion is gated by a check in the handler: the caller's session must hold a recent passkey step-up, and the caller must not be the member being promoted. The node's administration console depends on this path, stepping the session up and then sending the update. The same node also serves `acl/change-role`, the operation defined for role transitions, whose definition leaves any step-up for an elevated role to the maintainer. There, and on `acl/grant`, it assigns the administrative role to any administrative caller with no step-up, no refusal of self-promotion and without the role-change policy the update path applies. So the gate on the update path does not bound promotion at that node | Move promotion to `acl/change-role`. Require the step-up there, and on `acl/grant` for the administrative role, as a rule the policy gate evaluates rather than a check in a handler. Carry over what the update path does now: refuse self-promotion, serialise against concurrent role changes, and apply the role-change policy. Point the console at that operation, then refuse `role: admin` on `vtc/members/update` with `adminRoleForbidden`. The step-up on the role-change operation has to come first. Moving the console before it exists would remove the only gate that promotion has |
